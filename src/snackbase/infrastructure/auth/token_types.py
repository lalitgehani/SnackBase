...
class TokenPayload(BaseModel):
    ...
    scopes: list[str] = Field(
        ...
        description="API-key permission scopes (e.g. records:secrets:read)",
    )
    issued_at: int = Field(..., description="Unix timestamp when the token was issued")
    expires_at: int | None = Field(None, description="Unix timestamp when the token expires")
...
class AuthenticatedUser:
    ...
    def has_scope(self, scope: str) -> bool:
        """Return True if this principal was granted the given API-key scope."""
