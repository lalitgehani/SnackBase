...
async def create_invitation(
    ...
    # 2. Check if user already exists in account
    if await user_repo.email_exists_in_account(request.email, current_user.account_id):
        logger.info(
    ...
    if await invitation_repo.check_pending_invitation_exists(
        request.email, current_user.account_id
    ):
        logger.info(
...
async def list_invitations(
    ...
    """List invitations for the current user's account.

    Returns all invitations for the account, optionally filtered by status.
