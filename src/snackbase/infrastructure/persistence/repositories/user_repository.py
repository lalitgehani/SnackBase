...
class UserRepository:
    ...
    async def count_created_between(self, start: datetime, end: datetime) -> int:
        """Count users created in the half-open interval [start, end).
        ...
        Returns:
            Count of users created in the interval.
        """
        from sqlalchemy import func
    ...
    async def count_created_by_day(
        ...
        """Count users created per calendar day in [start, end).
        ...
        if dialect_name == "postgresql":
            day_expr = cast(UserModel.created_at, Date)
            day_label = cast(day_expr, String)
        else:
            day_label = func.strftime("%Y-%m-%d", UserModel.created_at)
            day_expr = day_label

        result = await self.session.execute(
            select(day_label, func.count(UserModel.id))
            .where(
                UserModel.created_at >= start,
                UserModel.created_at < end,
            )
