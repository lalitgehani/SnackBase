...
async def test_auth_flow_unified(client: AsyncClient, db_session: AsyncSession):
    ...
    from snackbase.infrastructure.auth.token_types import TokenPayload
    ...
    expired_payload = TokenPayload(
        ...
        role="admin",
        permissions=[],
        issued_at=int(datetime.now(UTC).timestamp()) - 3600,
        expires_at=int(datetime.now(UTC).timestamp()) - 10, # Expired
