...
class TestProviderRegistration:
    ...
    async def client(self, db_session):
        ...
        from snackbase.infrastructure.api.app import register_builtin_providers
        
        # Manually register providers since lifespan might not run or registry is cleared
        await register_builtin_providers(main_app)
        
        transport = ASGITransport(app=main_app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            # Force lifespan execution if it hasn't run yet
            await client.get("/health")
            yield client

    @pytest.mark.asyncio
    async def test_config_registry_exists_on_app_state(self, client):
        """Test that config_registry is created and stored on app.state."""
        from snackbase.infrastructure.api.app import app as main_app
        assert hasattr(main_app.state, "config_registry")
        assert main_app.state.config_registry is not None

    @pytest.mark.asyncio
    async def test_email_password_provider_registered(self, client):
    ...
    @pytest.mark.asyncio
    async def test_google_oauth_provider_registered(self, client):
    ...
    @pytest.mark.asyncio
    async def test_github_oauth_provider_registered(self, client):
    ...
    @pytest.mark.asyncio
    async def test_microsoft_oauth_provider_registered(self, client):
    ...
    @pytest.mark.asyncio
    async def test_apple_oauth_provider_registered(self, client):
    ...
    @pytest.mark.asyncio
    async def test_provider_appears_in_list(self, client):
    ...
    async def test_builtin_provider_cannot_be_deleted(self, db_session):
        ...
        if True: # Just use the provided db_session fixture
            session = db_session
            try:
